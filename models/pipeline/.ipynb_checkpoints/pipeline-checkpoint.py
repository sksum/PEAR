...
from main.metrics_head import calcu_mouth_open
import pickle
import random
import cv2
...
from evaluation_kp import smplx2smpl_joints
from main.evaluation import eval_pose
...
class OurPipeline():
    ...
    def run_fit(self, init_iter=0):
        ...
        for iter_idx in fit_bar:
            ...
            # HSMR loss weight  ; 
            # 1.  3D Loss
            # pd_vertices = smplx_dict['vertices']
            
            # loss_main, losses_main = self.compute_losses_main(self.loss_weight, outputs, batch)
            ...
            loss_dwpose_2d = self.metric( pred_kps2d[..., :2][kps2d_mask] / 1024,  batch['dwpose_kp2d'][..., :2][kps2d_mask] ) * 0.01   #  有 bug

            # loss_dwpose_2d = self.keypoint_2d_loss( pred_kps2d[..., :2] / 1024,   batch['dwpose_kp2d'] ) * 0.01   #  有 bug

            loss_dwpose_body_3d = self.keypoint_3d_loss(pred_kps3d[:,:24],  batch['dwpose_kp3d'][:,:24], pelvis_id=1 ) * 0.05    # 
            loss_dwpose_left_hand_3d = self.keypoint_3d_loss(pred_kps3d[:,92:113],  batch['dwpose_kp3d'][:,92:113], pelvis_id=0 ) * 0.01  
            loss_dwpose_right_hand_3d = self.keypoint_3d_loss(pred_kps3d[:,113:],  batch['dwpose_kp3d'][:,113:], pelvis_id=0 ) * 0.01   

            loss_dwpose_3d = loss_dwpose_body_3d + loss_dwpose_left_hand_3d + loss_dwpose_right_hand_3d
            ...
            # 4. Camera loss  丢掉
            # loss_camera =  self.camera_loss(outputs['pd_cam'][:,:3], batch['smplx_coeffs']['camera_RT_params']) * 0.01  # before 1
            ...
            # For test
            # bone_all = outputs['body_param']['body_pose'][0,[2,5,8],1].mean()
            # writer.add_scalar("369_bone", bone_all.item(), iter_idx)
            ...
            # # visualization
            batch_images = batch['image'] # 看一下弧度是不是特别大
            if iter_idx % 1000 == 0:
                with torch.no_grad():
                    rank = dist.get_rank() 
    ...
    def run_val(self, cur_iter,rank):
        ...
        # TODO: orient pose 差别很大， 骨骼点也是有旋转的吗？ Global pose 没有初始化好吗 ？    
        for batch in val_batches:
            ...
            for im_idx in img_indices:
                ...
                _t_lmk_dwp = pred_kps2d[im_idx,:,:2].detach().cpu().numpy()
                _landmark_dwp = batch['dwpose_rlt']['keypoints'][im_idx, ...].detach().cpu().numpy()
    ...
    def run_evaluation_body(self,val_name):
        val_iter = iter(self.val_dataloader)

        mpjpes, res, image_name, kp3d, kp2d = [], [], [], [], []
        valid_kp = [0,1,2,3,4,5,6,7,15,16,17,18,31,32,33,34,35,36,37,38,41,42,43]  # Only upbody
        for iter_idx in range(30000):
            try:
                batch = next(val_iter)
            except StopIteration:
                break

            img_patch = to_tensor(batch['ehm_image'], self.lightning_fabric.device)
            outputs = self.forward_step(img_patch)

            pd_dict = self.ehm(outputs['body_param'], outputs['flame_param'], pose_type='aa')
            gt_dict = self.ehm(batch['smplx_coeffs'], batch['flame_coeffs'], pose_type='aa')

            pred_joints = smplx2smpl_joints(pd_dict['vertices'])  # [B, 44, 3]
            gt_joints = smplx2smpl_joints(gt_dict['vertices'])    # [B, 44, 3]

            pred_kp3d = pred_joints.detach()        # [B, 1, 44, 3]
            gt_kp3d = gt_joints.expand_as( pred_kp3d )

            pred_kp3d -= pred_kp3d[:,  [41]]
            gt_kp3d -= gt_kp3d[:,  [41]]
            ...
            kp3d.append(pred_kp3d)
            # kp2d.append(batch['dwpose_rlt']['keypoints'])  # [B, 1, 134, 2] 
 
            # for idx ,img in enumerate(batch['image']): # For other method test
            #     torchvision.utils.save_image(img, f"/cto_studio/wujiahao/ehm_data/evaluation_dataset/our_evaluation_data/{val_name}/{batch['image_name'][idx]}.jpg"
            #                                   )

        mpjpes = np.concatenate(mpjpes).mean() # [N, 44, 3]
        res = np.concatenate(res).mean()
        flat_image_name = [img for batch_list in image_name for img in batch_list]
        flat_image_name = np.array(flat_image_name)
        kp3d = torch.cat(kp3d, dim=0)  # [N, 44, 3]
        # kp2d = torch.cat(kp2d, dim=0)  # [N, 44, 2] 

        # np.savez_compressed(f'/cto_studio/wujiahao/ehm_data/evaluation_dataset/our_evaluation_data/{val_name}.npz',
        #     imgname=flat_image_name,
        #     kp3d=kp3d.cpu().numpy(),
        # )

        # 打印结果
        print(f"Evaluation | MPJPE: {mpjpes:.4f} | RE: {res:.4f}")



    def run_evaluation_head(self,val_name):

        val_iter = iter(self.val_dataloader)
        save_dir = f"/cto_studio/wujiahao/ehm_data/evaluation_dataset/our_evaluation_data/{val_name}"
        our_head_save = f"outputs/our_evaluation_data/{val_name}"
        os.makedirs(save_dir, exist_ok=True)
        os.makedirs(our_head_save, exist_ok=True)
        with open('/cto_studio/vistring/liuyunfei/ws/Face3DRecons/smirk/assets/FLAME_masks/FLAME_masks.pkl', 'rb') as fin:
            masks = pickle.load(fin, encoding='latin1')
            mouth_map = masks['lips']
            upper_map = masks["forehead"].tolist() + masks["eye_region"].tolist()
            # upper_map = masks["forehead"].tolist()

        mean_distances, L2_dis_mouth_maxs, L2_dis_mouth_open_means = [], [], []
        with torch.no_grad():
            for iter_idx in range(300000):
                try:
                    batch = next(val_iter)
                except StopIteration:
                    break
                B = len(batch['ehm_image'])
                img_patch = to_tensor(batch['ehm_image'], self.lightning_fabric.device)
                outputs = self.forward_step(img_patch)
                ...
                pd_head_v = pd_dict['ori_head_vertices'][:,:-120]  # [B, 44, 3]
                gt_head_v = gt_dict['ori_head_vertices'][:,:-120]    # [B, 44, 3]

                for idx in range(B):
                    # np.save(
                    #     os.path.join(save_dir, f"{batch['image_name'][idx]}.npy"),
                    #     np.array(gt_head_v[idx].detach().cpu())
                    # )
                    np.save(
                        os.path.join(our_head_save, f"{batch['image_name'][idx]}.npy"),
                        np.array(pd_head_v[idx].detach().cpu())
                    )




        #         distances = torch.norm(pd_head_v - gt_head_v, dim=2)  # [B, 44]
        #         mean_distance = distances.mean()  # 标量


        #         # 假设 mouth_map 是一个 list 或 1D tensor，例如 [23, 24, 25, 26]
        #         mouth_indices = torch.tensor(mouth_map, device=gt_head_v.device)

        #         # 提取 mouth 顶点，形状为 [B, len(mouth_map), 3]
        #         gt_mouth = gt_head_v[:, mouth_indices, :]  # [B, M, 3]
        #         pd_mouth = pd_head_v[:, mouth_indices, :]  # [B, M, 3]

        #         # 计算逐点的 L2 距离平方：((gt - pd) ** 2).sum(dim=2) => [B, M]
        #         L2_dis_mouth_sq = (gt_mouth - pd_mouth).pow(2).sum(dim=2)

        #         # 每个样本 mouth 区域中最大的距离平方
        #         L2_dis_mouth_max = torch.mean(L2_dis_mouth_sq.max(dim=1).values)  # [B]

        #         L2_dis_mouth_open_mean = calcu_mouth_open(pd_head_v, gt_head_v, mouth_map)

        #         mean_distances.append(mean_distance)
        #         L2_dis_mouth_maxs.append(L2_dis_mouth_max)
        #         L2_dis_mouth_open_means.append(L2_dis_mouth_open_mean)



        # print('Mean Vertex Error: {:.4e}'.format(torch.mean(torch.stack(mean_distances))))
        # print('Lip Vertex Error: {:.4e}'.format(torch.mean(torch.stack(L2_dis_mouth_maxs))))
        # print(f'Mean Mouth Open Diference {torch.mean(torch.stack(L2_dis_mouth_open_means)):.4e}')
    ...
    #     print(f'Loading backbone weights from {self.cfg.backbone_ckpt}')
