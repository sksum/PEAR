...
class EHM_v2(nn.Module):
    ...
    def forward(self, body_param_dict:dict, flame_param_dict:dict=None,mano_param_dict:dict=None, zero_expression=False, zero_jaw=False, zero_shape=False,
        ...
        if flame_param_dict is not None:
            ...
            head_vertices, head_joints = lbs(betas, full_pose, template_vertices, # [1,5023,3]  [1,5,3]
            ...
            if head_scale is not None:   # 这样好像有点不align， 不能像手部那样吗？
        ...
        hand_scale        = body_param_dict['hand_scale']
        batch_size = shape_params.shape[0]
        ...
        if head_vertices is not None: # 把 smplx 的人头顶点替换成 Flame 估计的头
            ...
            selected_head[:, self.head_index] = head_vertices[:, self.head_index] - head_joints[:, 3:5].mean(dim=1, keepdim=True) \
                        + tbody_joints[:, 23:25].mean(dim=1, keepdim=True)
            new_template_vertices[:, self.smplx.smplx2flame_ind] = selected_head
        
        if hand_scale is not None:  # 这一段啥意思，为啥乘以 hand scale
        ...
        if self.smplx.use_joint_regressor:  # True
            reg_joints = torch.einsum(
                'ji,bik->bjk', self.smplx.extra_joint_regressor, vertices) # [B,14,3]

            joints[:, self.smplx.source_idxs.long()] = (  # 原本的版本
                joints[:, self.smplx.source_idxs.long()].detach() * 0.0 +
                reg_joints[:, self.smplx.target_idxs.long()] * 1.0
            )

            # new_joints = joints.clone()  # 改后的版本
            # new_joints[:, self.smplx.source_idxs.long()] = (
            #     reg_joints[:, self.smplx.target_idxs.long()]
            # )
            # joints = new_joints
...
if __name__ == "__main__":
    ...
    flame_assets_dir = "/cto_labs/zhangdongbin/code/Gen-Ubody-Avatar/assets/FLAME"  # 替换为你的 FLAME 资产目录
    smplx_assets_dir = "/cto_labs/zhangdongbin/code/Gen-Ubody-Avatar/assets/SMPLX"  # 替换为你的 SMPLX 资产目录
    mano_assets_dir = "/cto_labs/zhangdongbin/code/Gen-Ubody-Avatar/assets/MANO"  # 替换为你的 MANO 资产目录
    ehm_model = EHM_v2(flame_assets_dir, smplx_assets_dir, mano_assets_dir,add_teeth=True).to(device)
    ...
    body_param_dict = {
        ...
        #'head_pos_offset': torch.randn(batch_size,3).to(device),
        #'left_hand_scale': torch.randn(batch_size).to(device),
        #'left_hand_pos_offset': torch.randn(batch_size,3).to(device),
        #'right_hand_scale': torch.randn(batch_size).to(device),
        #'right_hand_pos_offset': torch.randn(batch_size,3).to(device),
    }
    flame_param_dict = {
    ...
    save_obj(f'/cto_labs/zhangdongbin/code/Gen-Ubody-Avatar/z_temp/output_ehm_head.obj', vertices, faces)